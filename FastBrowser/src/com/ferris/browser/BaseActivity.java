...
import com.ferris.browser.constant.BookmarkPage;
...
public class BaseActivity extends FragmentActivity implements
	...
	private static final FrameLayout.LayoutParams COVER_SCREEN_PARAMS = new FrameLayout.LayoutParams(
			LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT);
	protected View mTitleBar, bottom_bar;
	...
	private synchronized void initialize() {
		...
		if (Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP) {
			...
			mDeleteIcon = getResources().getDrawable(
					R.drawable.ic_action_delete, theme);
			mRefreshIcon = getResources().getDrawable(
					R.drawable.ic_action_refresh, theme);
			mCopyIcon = getResources().getDrawable(R.drawable.ic_action_copy,
					theme);
		}
	...
	private void initializeProxy() {
		...
		try {
			WebkitProxy.setProxy(BrowserApp.class.getName(),
	...
	protected synchronized boolean newTab(String url, boolean show) {
		...
		mIsNewIntent = false;
		FastView startingTab = new FastView(mActivity, "http://www.qq.com", mDarkTheme);
		startingTab.setHeadorFooderView(mTitleBar, bottom_bar);
		if (mIdGenerator == 0) {
